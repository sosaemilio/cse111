"""Emilio Sosa -  BYUI Programming with Functions
Instructor: Brother Craig Lindstrom
...
def main():
    ...
    formula = parse_formula(chemical_formula, periodic_table_dic)

    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    molar_mass = compute_molar_mass(formula, periodic_table_dic)

    # Compute the number of moles in the sample.
    moles = computer_moles(mass, molar_mass)
    ...
    # Create Chemical Formulas Names Dictionary and store it on variable
    known_molecules_dict = make_formulas_name_dic()
...
def make_periodic_table():
    """ Creates the periodic table and returns it.
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for formula in symbol_quantity_list:

        #Separate the inner list into symbol and quantity
        symbol = formula[SYMBOL_INDEX]
        quantity = formula[QUANTITY_INDEX]

        # Obtain the new "list" from the dictonary from key named "symbol" and stored in variable name element
        element = periodic_table_dict[symbol]

        # Obtain the atomic mass of the element
...
def computer_moles(mass, molar_mass):
    """Computes the molar formula
    Parameters:
        mass is the mass of the element in grams.
        molar_mass is the molar mass obtained from the function compute_molar mass.
    Return
        moles
    """
